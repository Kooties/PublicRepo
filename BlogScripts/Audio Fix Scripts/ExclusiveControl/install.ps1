<#Check for Staging folder; create it if it's not there, then move the remediation script there#>
...
if(Test-Path -Path "C:\Staging"){
    Copy-Item -Path .\audioExclusiveControlFix.ps1 -Destination c:\Staging\audioExclusiveControlFix.ps1 -Force
}else{
    New-Item -path "C:\Staging" -ItemType Directory -Force
    $folder = Get-Item 'C:\Staging' -Force
    $folder.Attributes='Hidden'
    Copy-Item -Path .\audioExclusiveControlFix.ps1 -Destination c:\Staging\audioExclusiveControlFix.ps1 -Force
}

$scriptFile = "-ExecutionPolicy Unrestricted -File c:\Staging\audioExclusiveControlFix.ps1"
$action = New-ScheduledTaskAction -Execute %SystemRoot%\system32\WindowsPowerShell\v1.0\powershell.exe -Argument $scriptFile
$trigger = New-ScheduledTaskTrigger -AtStartup
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries
$task = New-ScheduledTask -Action $action -Trigger $trigger -Settings $settings
Register-ScheduledTask -TaskName "Fix Exclusive Audio Control" -InputObject $task -User "System"
